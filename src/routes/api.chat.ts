...
async function anthropicUsable(_key: string): Promise<boolean> {
  return (await anthropicHealth()).ok;
}
...
- USER-DRAWN LEVELS WIN (highest priority): if the image contains drawn lines, boxes, labels, order tickets, position overlays, or entry/SL/TP text - or the trader names an entry in the message - use THOSE exact numbers verbatim for entry (and stop/TP where drawn). Do NOT substitute your own entry, do NOT "improve" it, and do NOT reuse an entry from any earlier scan of this instrument in this thread. Earlier scan levels are historical reference only; the uploaded chart and the trader's own numbers override them completely. If you believe their entry is worse than another level, still grade THEIR entry, then mention the alternative in one clause.
- Only when no entry is drawn or stated: derive entry from visible structure - order blocks, fair value gaps, swing highs/lows, liquidity pools, trendlines, moving averages, session opens. Place stop beyond the invalidation structure (not a fixed pip/percent from price). Place TP1/TP2 at the next liquidity or structural targets visible in the image.
    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          const block = await measuredHitRatePrompt(sym, userId ?? undefined);
