...
function FinalExamPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {submitted && (
        <div className={`mb-5 rounded-xl border p-5 ${passed ? "border-primary bg-primary/10" : "border-destructive/40 bg-destructive/5"}`}>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {passed && (
              <Link
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold bg-primary text-primary-foreground"
              >
            ...
            <button
              onClick={reset}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium border border-border/60 bg-card"
            >
      ...
      <div className="space-y-3">
        {exam.map((item, i) => {
          ...
          return (
            <div key={`${item.moduleId}-${item.qIndex}`} className="rounded-xl border border-border/60 bg-card p-4">
                ...
                {item.q.choices.map((c, j) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-left text-sm px-3 py-2 rounded-xl border transition ${cls}`}
                    >
                      {c}
                    </button>
                  );
      ...
      {!submitted && (
        <div className="sticky bottom-4 mt-6 flex items-center justify-between gap-3 rounded-xl border border-border/60 bg-card p-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold bg-primary text-primary-foreground disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Submit exam
          </button>
        </div>
