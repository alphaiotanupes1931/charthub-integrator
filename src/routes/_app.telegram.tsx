...
export const Route = createFileRoute("/_app/telegram")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Link your Telegram account so TradeMind sends morning briefings, evening reports and high conviction signals straight to your chat." },
      { property: "og:title", content: "Telegram delivery, TradeMind" },
      { property: "og:description", content: "Link Telegram to receive TradeMind briefings and signals in your chat." },
...
function TelegramPage() {
  ...
  const sendNow = useMutation({
    ...
    onSuccess: (r: any) => toast.success(r.delivered ? "Sent to Telegram" : "Saved in app. Telegram is not linked yet."),
    onError: (e: Error) => toast.error(e.message),
  });

  if (state.isLoading) return <div className="p-6 text-sm text-muted-foreground">Loading Telegram settings...</div>;
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 md:p-6 space-y-6">
        ...
        {prefs.telegram_chat_id ? (
          <div className="flex items-center justify-between gap-2">
            <div className="text-sm">
              Linked to chat <code className="rounded bg-muted px-1.5 py-0.5 text-xs">{prefs.telegram_chat_id}</code>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => { navigator.clipboard.writeText(`/start ${prefs.telegram_link_code}`); toast.success("Copied"); }}
            ...
            <p className="text-xs text-muted-foreground">Refresh this page once you have sent it.</p>
          </div>
        ) : (
      ...
      <section className="rounded-md border border-border bg-card/40 p-4 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">Sends a briefing right now using your watchlist and today's economic calendar.</p>
        <button
        ...
        ><Send className="size-4" /> {sendNow.isPending ? "Sending..." : "Send a test briefing"}</button>
      </section>

      <p className="text-sm text-muted-foreground">
        Schedule, timezone and watchlist live in <Link to="/briefings" className="underline">Briefings</Link>. The session news read lives in{" "}
