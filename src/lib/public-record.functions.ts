  ...
  .handler(async ({ data }): Promise<PublicRecord> => {
    ...
    return {
      ...
      caveats: [
        ...
        "Signals filed before 18 September 2026 were published without any check that the entry price was still reachable. Measured on 500 of them, price had already closed past the planned entry on 418, by a median of 0.69 of the trade's risk, and 145 resolved on the very first candle. From that date a signal is refused if price already sits more than 0.5 of its risk past its own entry, and the distance is stored on every signal filed.",
        "Because of that change, figures from before and after 18 September 2026 are not the same measurement and should not be pooled. Re-scored on entries that were still reachable at filing time, the older signals average -0.15R to +0.16R depending on how strict the reachability test is, which is materially worse than the headline figures for the same period.",
        "How long a signal may run before it expires is now set per market from its own measured time to resolution, replacing a single clock applied to every instrument. Signals with too small a sample keep the old conservative clock. Same-bar fills are still resolved pessimistically: a candle holding both the stop and the target counts as a stop.",
